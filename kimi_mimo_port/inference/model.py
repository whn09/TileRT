...
class ModelArgs:
    ...
    index_topk: int = 2048
    # DSA indexer toggle: True for DeepSeek-V3.2 (sparse), False for standard
    # MLA models like Kimi-K2 (full attention, no indexer weights in checkpoint)
...
class MLA(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.use_indexer = getattr(args, "use_indexer", True)
        self.indexer = Indexer(args) if self.use_indexer else None
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor,
        ...
        if mask is not None:  # MHA prefill
            ...
            if self.use_indexer:
                # DSA sparse indexer (DeepSeek-V3.2)
            ...
            if self.use_indexer:
                ...
                index_mask = torch.full((bsz, 1, end_pos), float("-inf"),
                                        device=x.device).scatter_(-1, topk_indices, 0)
                scores += index_mask.unsqueeze(2)
            # else: standard full MLA decode (Kimi-K2) attends to all positions
